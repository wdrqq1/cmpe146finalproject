...
PRIVILEGED_DATA        volatile portTickType xTickCount 						= ( portTickType ) 0;
...
#if ( configGENERATE_RUN_TIME_STATS == 1 )
	unsigned long ulISROverhead = 0UL;
	// PRIVILEGED_DATA static char pcStatsString[ 50 ] ;
	PRIVILEGED_DATA static unsigned long ulTaskSwitchedInTime = 0UL;	/*< Holds the value of a timer/counter the last time a task was switched in. */
	static unsigned long prvGenerateRunTimeStatsForTasksInList( char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime, char resetCounters ) PRIVILEGED_FUNCTION;
...
/*-----------------------------------------------------------*/
	...
	unsigned portBASE_TYPE uxQueue;
		...
		vTaskSuspendAll();
		{
			...
			siprintf((char*)pcWriteBuffer, "%16s   ST  PR  ST Free  Task#\r\n", "Task Name");
...
/*----------------------------------------------------------*/
	...
	unsigned int uxTaskGetIdlePercentage(char resetCounters)
	{
		...
		vTaskSuspendAll();
		xList *pxList = ( xList * ) &( pxReadyTasksLists[ 0 ] );
		...
		do
		{
			...
			if(pxNextTCB->pcTaskName[0] == 'I' && pxNextTCB->pcTaskName[1] == 'D' &&
				pxNextTCB->pcTaskName[2] == 'L' && pxNextTCB->pcTaskName[3] == 'E')
			{
				...
				if( pxNextTCB->ulRunTimeCounter == 0 ) {
					...
					ulStatsAsPercentage = ( (long long)100 * pxNextTCB->ulRunTimeCounter ) / ulTotalRunTime;
				}
				break;
			}
	...
	void runTimeStatISREntry()  __attribute__ ((always_inline));
	void runTimeStatISREntry()
	{
			// Get current time
	...
	void runTimeStatISRExit() __attribute__ ((always_inline));
	void runTimeStatISRExit()
	{
			// Log how long ISR took
			// Get current time
			unsigned long ulTempCounter = portGET_RUN_TIME_COUNTER_VALUE();
	...
	void vTaskGetRunTimeStats( char *pcWriteBuffer, char resetCounters )
	{
		unsigned portBASE_TYPE uxQueue;
		unsigned long ulTotalRunTime = portGET_RUN_TIME_COUNTER_VALUE();
		unsigned long runTimeCounterOfAllTasks = 0;
		...
		vTaskSuspendAll();
		{
			...
			do
			{
				...
				if( !listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) )
				{
					runTimeCounterOfAllTasks += prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), ulTotalRunTime, resetCounters);
				}
			}while( uxQueue > ( unsigned short ) tskIDLE_PRIORITY );

			if( !listLIST_IS_EMPTY( pxDelayedTaskList ) )
			{
				runTimeCounterOfAllTasks += prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) pxDelayedTaskList, ulTotalRunTime, resetCounters );
			}

			if( !listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) )
			{
				runTimeCounterOfAllTasks += prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) pxOverflowDelayedTaskList, ulTotalRunTime, resetCounters );
			}
			...
			#if ( INCLUDE_vTaskDelete == 1 )
			{
				if( !listLIST_IS_EMPTY( &xTasksWaitingTermination ) )
				{
					runTimeCounterOfAllTasks += prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) &xTasksWaitingTermination, ulTotalRunTime, resetCounters );
				}
			}
			#endif
			...
			#if ( INCLUDE_vTaskSuspend == 1 )
			{
				if( !listLIST_IS_EMPTY( &xSuspendedTaskList ) )
				{
					runTimeCounterOfAllTasks += prvGenerateRunTimeStatsForTasksInList( pcWriteBuffer, ( xList * ) &xSuspendedTaskList, ulTotalRunTime, resetCounters );
				}
			}
			#endif
		}

		unsigned int overheadPercent = ((unsigned long long)ulISROverhead*100)/ulTotalRunTime ;
		if(overheadPercent < 1) {
			siprintf( pcWriteBuffer + strlen(pcWriteBuffer), ( char * ) "%16s  %10u   <1%%\r\n", "<OVERHEAD>", (unsigned int)ulISROverhead);
		}
		else {
			siprintf( pcWriteBuffer + strlen(pcWriteBuffer), ( char * ) "%16s  %10u  %3u%%\r\n", "<OVERHEAD>", (unsigned int)ulISROverhead, overheadPercent);
		}

		if(resetCounters) {
			ulISROverhead = 0UL;
			ulTaskSwitchedInTime = 0UL;
			portRESET_RUN_TIME_COUNTER();
		}
		xTaskResumeAll();
	}
...
/*-----------------------------------------------------------*/
	...
	unsigned short usStackRemaining;
		...
		do
		{
			...
			siprintf( (char*)(pcWriteBuffer + strlen((char*)pcWriteBuffer)),
						"%16s    %c  %u    %4u    %u\r\n",
						pxNextTCB->pcTaskName,
						cStatus,
						( unsigned int ) pxNextTCB->uxPriority,
						usStackRemaining, ( unsigned int )
...
/*-----------------------------------------------------------*/
	...
	static unsigned long prvGenerateRunTimeStatsForTasksInList( char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime, char resetCounter )
	{
		volatile tskTCB *pxNextTCB, *pxFirstTCB;
		...
		do
		{
			...
			if( ulTotalRunTime > 0UL )
			{
				...
				if( pxNextTCB->ulRunTimeCounter == 0 )
				{
					...
					siprintf( pcWriteBuffer + strlen(pcWriteBuffer), ( char * ) "%16s  %10u    0%%\r\n", pxNextTCB->pcTaskName, 0 );
				}
				else
				{
					totalRunTime += pxNextTCB->ulRunTimeCounter;
					...
					if(resetCounter) {
						 pxNextTCB->ulRunTimeCounter =  0UL;
					}
				}
			}
